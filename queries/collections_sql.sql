...
insert into collections (query_name, value) values ('art_gallery_points', (select count(*) from planet_osm_point where (tourism in ('gallery', 'art_gallery', 'Art Gallery', 'Art_gallery') or amenity in ('gallery', 'art_gallery', 'arts_gallery', 'contemporary_art_gallery') or (tags ?| array ['museum','gallery'])) or building in ('gallery', 'art_gallery') and (osm_id > 0)));
...
-- QUERIES FOR LIBRARIES (add museum=gallery)
...
-- QUERIES FOR MUSEUMS (add museum=* w/o gallery)

-- How many museum points are there?
insert into collections (query_name, value) values ('museum_points', (select count(*) from planet_osm_point where (tourism = 'museum' or amenity = 'museum' or building = 'museum' or tags? 'museum') and (osm_id > 0)));

-- How many museum polygons are there?
insert into collections (query_name, value) values ('museum_polygons', (select count(*) from planet_osm_polygon where (tourism = 'museum' or amenity = 'museum' or building = 'museum' or tags? 'museum') and (osm_id > 0)));
