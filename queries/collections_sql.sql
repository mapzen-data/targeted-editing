...
insert into collections (query_name, value) values ('library_points', (select count(*) from planet_osm_point where (amenity = 'library' or building = 'library' or tags? 'library' or tags @> '"museum"=>"gallery"'::hstore) and (osm_id > 0)));

-- How many library polygons are there?
insert into collections (query_name, value) values ('library_polygons', (select count(*) from planet_osm_polygon where (amenity = 'library' or building = 'library' or tags? 'library' or tags @> '"museum"=>"gallery"'::hstore) and (osm_id > 0)));
