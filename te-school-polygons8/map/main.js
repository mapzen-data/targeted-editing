...
//         });
        ...
        map.getContainer().addEventListener('mousemove', function (event) {
            ...
            scene.getFeatureAt(pixel).then(function(selection) {
                ...
                if ( (scene.selection.feature.properties.kind == 'school' ||
                      scene.selection.feature.properties.kind == 'kindergarten' ||
                      scene.selection.feature.properties.kind == 'university')
                   && !scene.selection.feature.properties.area ) 
                {
	                popup.style.visibility = 'visible';
	            }
                popup.innerHTML = '<span class="labelInner">' + 'You found a school that needs help!' + '</span><br>';
    ...
    function createEditLinkElement (url, type, label) {
        ...
        anchor.addEventListener('click', function (event) {
            trackOutboundLink(url, 'editing_schools', type);
        }, false);
